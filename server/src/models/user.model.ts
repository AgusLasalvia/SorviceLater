...
export class UserModel {
	...
	static async AddUser(user: User): Promise<User | null> {
		const db = await Database.getInstance();
		const sql = `INSERT INTO User (firstName,lastName,email,password,createdAt,updatedAt) VALUES (?,?,?,?,?,?)`;
		const [result] = await db.query(sql, [user.firstName, user.lastName, user.email, user.password, new Date(), new Date()]);
		return result ? user : null;
	}
}